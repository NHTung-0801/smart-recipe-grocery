...
# Phần Backend_Server
```text
smart-recipe-microservices/
│
├── docker-compose.yml              # Cấu hình khởi chạy toàn bộ hệ thống (App + MySQL + Redis + MinIO)
├── docker-compose.dev.yml          # Cấu hình override cho môi trường dev (ví dụ map port local, hot-reload)
├── .env.example                    # Chứa các biến môi trường mẫu dùng chung
├── Makefile                        # Chứa các script chạy nhanh (vd: make up, make migrate, make test)
├── README.md                       
│
├── infrastructure/                 # (Tùy chọn) Chứa script khởi tạo hạ tầng nếu cần
│   ├── mysql/                      # Script init.sql để tạo các database riêng biệt cho từng service
│   └── minio/                      # Script tạo bucket ban đầu
│
├── gateway-service/                # 🚪 ENTRY POINT: Cổng giao tiếp duy nhất nhận request từ Android
│   ├── Dockerfile
│   ├── requirements.txt
│   └── app/
│       ├── main.py
│       ├── api/
│       │   └── router.py           # Định tuyến (Proxy) request đến các service nội bộ tương ứng
│       ├── middleware/
│       │   ├── auth_middleware.py  # Verify JWT Token tại cổng, sau đó inject `user_id` vào Header
│       │   ├── rate_limiter.py     # Giới hạn số lượng request bằng Redis để chống spam
│       │   └── logging_middleware.py
│       └── core/
│           ├── config.py           # Khai báo URL của các service nội bộ (SERVICE_URLS)
│           └── http_client.py      # Quản lý connection pool (httpx AsyncClient) để gọi các service khác
│
├── auth-service/                   # 👤 HỆ THỐNG NGƯỜI DÙNG & BẢO MẬT
│   ├── Dockerfile, requirements.txt, alembic/, tests/
│   └── app/
│       ├── main.py
│       ├── api/v1/
│       │   ├── auth.py             # Endpoint: /register, /login, /refresh, /logout
│       │   └── profile.py          # Endpoint: GET/PUT /me
│       ├── core/
│           ├── security.py         # Logic mã hóa/giải mã JWT, hash mật khẩu bằng bcrypt
│           └── config.py
│       ├── db/                     
│       │   ├── database.py         # Cấu hình AsyncSession kết nối DB
│       │   └── deps.py             # Hàm cung cấp get_db
│       ├── models/                 
│       │   └── user.py             # SQLAlchemy Model: Bảng `users`
│       ├── schemas/                
│       │   └── auth.py             # Pydantic: LoginRequest, TokenResponse...
│       └── services/               
│           └── auth_service.py     # Business logic xử lý đăng nhập, cấp token
│
├── recipe-service/                 # 🍲 QUẢN LÝ CÔNG THỨC & TÍNH NĂNG CỐT LÕI
│   ├── Dockerfile, requirements.txt, alembic/, tests/
│   └── app/
│       ├── main.py
│       ├── api/v1/
│       │   ├── recipes.py          # CRUD công thức
│       │   ├── ingredients.py      # Quản lý nguyên liệu của công thức
│       │   ├── tags.py             # Quản lý và gợi ý thẻ tag
│       │   ├── grocery.py          # Logic đi chợ: extract, aggregate, categorize
│       │   ├── planner.py          # Lập lịch thực đơn
│       │   └── journal.py          # Nhật ký nấu ăn
│       ├── db/, core/              # Cấu trúc tương tự auth-service
│       ├── models/                 # SQLAlchemy: `recipes`, `ingredients`, `tags`, `meal_plans`...
│       ├── schemas/                # Pydantic validation
│       └── services/               
│           ├── recipe_service.py
│           ├── grocery_service.py  # Chứa thuật toán gộp nguyên liệu phức tạp
│           └── planner_service.py
│
├── social-service/                 # 🌐 CỘNG ĐỒNG GIAO LƯU (Sử dụng MySQL + Redis)
│   ├── Dockerfile, requirements.txt, alembic/, tests/
│   └── app/
│       ├── main.py
│       ├── api/v1/
│       │   ├── feed.py             # Load Newsfeed (Kết hợp Paging và Redis cache)
│       │   ├── posts.py            # Publish công thức lên server
│       │   ├── likes.py            # Thả tim bài viết
│       │   ├── comments.py         # Bình luận
│       │   └── clone.py            # Logic tải thẳng công thức về máy (trả về RecipeDto)
│       ├── models/, schemas/, db/, core/
│       └── services/               
│           ├── feed_service.py     # Logic tối ưu cache Redis cho Newsfeed
│           └── social_service.py
│
├── ai-service/                     # 🤖 TRỢ LÝ AI (Tích hợp Gemini API)
│   ├── Dockerfile, requirements.txt, tests/
│   └── app/
│       ├── main.py
│       ├── api/v1/
│       │   ├── generate.py         # Endpoint tạo công thức từ mô tả
│       │   └── zero_waste.py       # Giải cứu tủ lạnh
│       ├── core/
│       │   ├── gemini_client.py    # Wrapper đóng gói google-generativeai
│       │   └── config.py
│       ├── prompts/                # Quản lý các system prompt
│       │   ├── generate_prompt.py  # Prompt ép AI trả về chuẩn JSON mong muốn
│       │   └── zero_waste_prompt.py
│       ├── schemas/                
│       │   ├── ai_request.py       # Pydantic: Định dạng request đẩy lên
│       │   └── recipe_output.py    # Pydantic: Chuẩn hóa JSON từ AI thành RecipeDto
│       └── services/               
│           └── ai_service.py       # Lắp ghép prompt, gọi LLM, parse và validate dữ liệu
│
└── media-service/                  # 🖼️ LƯU TRỮ HÌNH ẢNH (Sử dụng MinIO/S3)
    ├── Dockerfile, requirements.txt, tests/
    └── app/
        ├── main.py
        ├── api/v1/
        │   └── upload.py           # Endpoint nhận file multipart/form-data
        ├── core/
        │   ├── storage.py          # Logic khởi tạo S3 client, tạo presigned URL
        │   └── config.py
        ├── schemas/
        │   └── media.py            # UploadResponse { url, key }
        └── services/
            └── media_service.py    # Xử lý ảnh: resize, nén, validate MIME type trước khi lưu
